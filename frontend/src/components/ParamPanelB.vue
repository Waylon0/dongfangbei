        ...
        <div class="param-grid">
          <template v-for="key in group.keys" :key="key">
            <div v-if="BOOLEAN_KEYS.has(key)" class="param-row">
              ...
              <el-switch
                :model-value="Boolean((params as any)[key])"
                @update:model-value="(val: boolean) => (params as any)[key] = val"
                size="small"
              />
            </div>
            <div v-else class="param-row">
              <span class="param-label">{{ LABELS[key] }}</span>
              <el-input-number
                :model-value="Number((params as any)[key])"
                @update:model-value="(val: number | undefined) => (params as any)[key] = val"
...
import { DEFAULT_PARAMS, PARAM_GROUPS, type PipelineParams } from '@/types'
