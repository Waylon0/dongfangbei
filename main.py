...
def generate_synthetic_data(shape: tuple = (300, 400),
    ...
    """生成合成测试数据：包含线状的断层高值区域。

    断层表现为有一定宽度的线性高值条带。
    """
    rng = np.random.default_rng(seed)
    data = np.full(shape, 0.05)  # 低背景值
    ...
    for _ in range(n_faults):
        ...
        _draw_thick_line(data, r1, c1, r2, c2,
                          strength=rng.uniform(0.6, 0.95),
                          width=rng.integers(4, 10))

        # 随机添加分支
        if rng.random() > 0.5:
            mr = (r1 + r2) // 2 + rng.integers(-30, 30)
            mc = (c1 + c2) // 2 + rng.integers(-30, 30)
            _draw_thick_line(data, (r1 + r2) // 2, (c1 + c2) // 2,
                              mr, mc, strength=rng.uniform(0.4, 0.7),
